...
const pat = 'http://localhost:3000';//'http://10.51.4.100:3000';
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Future<void> _searchUser() async{
    ...
    final url = Uri.parse('$pat/utilisateurs/${temp.username}');//10.0.2.2//10.51.4.100//10.52.4.1
    print(url);
...
class _PageAccueilState extends State<PageAccueil> {
  ...
  Future<void> _searchProduct() async{
    final url = Uri.parse('$pat/produits/${_productname.text}');//10.0.2.2:3000/produits/${_productname.text}');//10.0.2.2//10.51.4.100//10.52.4.1
    print(url);
    final response = await http.get(url);
    if (response.statusCode == 200){
      print('Aucune erreur : ${response.statusCode}');
...
class _ProduitDetailScreenState extends State<ProduitDetailScreen>{
  ...
  Future<void> updateProduit(int stock) async {
    //si les champs textes ne sont pas vides alors
    final url = Uri.parse('$pat/change_stock_produit');
    final headers = {'Content-Type': 'application/json'};
    setState(() {
...
class _AjoutProduitDetailScreenState extends State<AjoutProduitDetailScreen>{
  ...
  Future<void> ajouterProduit(Produit pro) async {
    //si les champs textes ne sont pas vides alors
    final url = Uri.parse('$pat/insert_or_update_produit');
    final headers = {'Content-Type': 'application/json'};
    final body = json.encode(pro.toMap());
    try {
      final response = await http.post(
